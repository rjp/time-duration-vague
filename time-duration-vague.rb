		...
		def Duration.vague(x)
			...
			[
				...
				if (units > 0) or (units == 0 and pcage > 85) then
					d = case
						when pcage < 10: "about #{units} #{name}"
						when pcage < 20: "just over #{units} #{name}"
                        when pcage < 40: "more than #{units} #{name}"
                        when pcage < 60: "about #{units} and a half #{name}"
                        when pcage < 80: "closer to #{next_min} #{name}"
						when pcage < 90: "almost #{next_min} #{name}"
                        when pcage >= 90: "about #{next_min} #{name}"
					end
